    ...
    <div className="min-h-screen w-[33%] bg-[#0B0B0B] py-30 flex flex-col justify-center items-start gap-20">
      ...
      <div className="flex flex-col justify-center items-start gap-10">
        <div className="flex flex-col justify-start items-start gap-6 px-20">
          ...
          {formData.botName ? (
            <p className="text-[18px] h-[31px] whitespace-nowrap text-ellipsis overflow-hidden w-[65%] ">
              {formData.botName}
            </p>
          ) : (
            ...
            <div className="flex flex-col gap-2">
              {formData.files.map((file, i) => (
                <div key={i} className="flex gap-2 items-center ">
                  ...
                  />
                  {/* )} */}
                  <p className="whitespace-nowrap text-ellipsis overflow-hidden w-[65%] ">
                    {file.name}
                  </p>
                </div>
              ))}
        ...
        <div className="flex flex-col justify-start items-start gap-6 px-20">
          ...
          <div className="flex flex-col gap-2 whitespace-nowrap text-ellipsis overflow-hidden w-[65%]">
            {formData.files.length > 0 || formData.urls.length > 0 ? (
              <p>{formData.botName} Bot is ready to go Live! 🚀</p>
            ) : (
              <div className="w-[341px] h-[31px] bg-[#1E1E1E] rounded"></div>
            )}
          </div>
